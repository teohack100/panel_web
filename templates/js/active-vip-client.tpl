...
var activeVipCountdownReloadPending = false;

function scheduleActiveVipCountdownReload() {
	if (activeVipCountdownReloadPending) {
		return;
	}
	activeVipCountdownReloadPending = true;
	setTimeout(function() {
		if ($.fn.DataTable.isDataTable('#users-serverside')) {
			$('#users-serverside').DataTable().ajax.reload(null, false);
		}
		activeVipCountdownReloadPending = false;
	}, 1500);
}
	...
	$('#users-serverside .live-countdown').each(function() {
		...
		if (decrement > 0 && current > 0) {
			...
			if (previous > 0 && current === 0) {
				scheduleActiveVipCountdownReload();
			}
		}
...
$('document').ready(function()
{
	table = $('#users-serverside').dataTable({
        ...
        "bStateSave": false,
        "ajax": {
            "url": "{$base_url}index.php?p=active-vip-serverside",
            "type": "POST"
        },
		"aoColumnDefs": [{
			'bSortable': false,
