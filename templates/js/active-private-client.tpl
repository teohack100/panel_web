...
function formatActivePrivateDuration(totalSeconds) {
...
var activePrivateCountdownReloadPending = false;

function scheduleActivePrivateCountdownReload() {
	if (activePrivateCountdownReloadPending) {
		return;
	}
	activePrivateCountdownReloadPending = true;
	setTimeout(function() {
		if ($.fn.DataTable.isDataTable('#users-serverside')) {
			$('#users-serverside').DataTable().ajax.reload(null, false);
		}
		activePrivateCountdownReloadPending = false;
	}, 1500);
}

function renderActivePrivateCountdowns(step) {
	...
	$('#users-serverside .live-countdown').each(function() {
		...
		if (decrement > 0 && current > 0) {
			...
			if (previous > 0 && current === 0) {
				scheduleActivePrivateCountdownReload();
			}
		}
		$el.html(formatActivePrivateDuration(current));
	});
}

setInterval(function() {
	renderActivePrivateCountdowns(1);
}, 1000);
...
$('document').ready(function()
{
	table = $('#users-serverside').dataTable({
        ...
        "bStateSave": false,
        "ajax": {
            "url": "{$base_url}index.php?p=active-private-serverside",
            "type": "POST"
        },
		"aoColumnDefs": [{
			'bSortable': false,
		...
		"drawCallback": function() {
			renderActivePrivateCountdowns(0);
		}
	});
