...
<head>
    ...
    <title>{$siteTitle} - Iniciar sesion</title>
    {assign var=pm_login_favicon value="`$base_url`logo/favicon2.png?v=2"}
    {if isset($panel_favicon_url) && $panel_favicon_url neq ''}
        {assign var=pm_login_favicon value=$panel_favicon_url}
    {/if}
    <link rel="icon" type="image/png" href="{$pm_login_favicon}">
    <link rel="shortcut icon" type="image/png" href="{$pm_login_favicon}">
...
<body>
    <div class="auth-card">
        {assign var=pm_login_logo value=''}
        {if isset($panel_login_logo_url) && $panel_login_logo_url neq ''}
            {assign var=pm_login_logo value=$panel_login_logo_url}
        {elseif isset($panel_logo_url) && $panel_logo_url neq ''}
            {assign var=pm_login_logo value=$panel_logo_url}
        {/if}
        {if $pm_login_logo neq ''}
        <div class="login-logo-wrap">
            <img src="{$pm_login_logo}" alt="Logo panel">
        </div>
        {/if}
        <h1 class="auth-title">Iniciar sesion</h1>
        {if $control_allow_register eq 1}
        <p class="auth-sub">¿Nuevo usuario? <a href="{$base_url}index.php?p=register">Crear una cuenta</a></p>
        {else}
        <p class="auth-sub">Acceso solo para cuentas autorizadas</p>
        {/if}

        {if $magic_notice_text ne ''}
        <div class="pm-alert pm-alert-{$magic_notice_class}">{$magic_notice_text}</div>
        {/if}
        ...
        <div class="social-stack">
            <a class="social-btn social-google" href="{$base_url}index.php?p=social-start&provider=google"><span class="google-g-icon" aria-hidden="true"></span> Continuar con Google</a>
            <a class="social-btn social-facebook" href="{$base_url}index.php?p=social-start&provider=facebook"><i class="fa-brands fa-facebook-f"></i> Continuar con Facebook</a>
            <a class="social-btn social-apple" href="{$base_url}index.php?p=social-start&provider=apple"><i class="fa-brands fa-apple"></i> Continuar con Apple</a>
        </div>
    ...
    <script>
        (function(){
            ...
            var postLoginRedirect = '{$post_login_redirect|default:"`$base_url`index.php?p=dashboard"}';
            ...
            $('#magic-link-btn').on('click', function(){
                ...
                $.ajax({
                    type: 'POST',
                    url: '{$base_url}serverside/forms/magic_link.php',
