...
{
	if(!isset($_POST['username']) && !isset($_POST['slot_code']) && !isset($_POST['slot'])
		|| empty($_POST['username']) || empty($_POST['slot_code']) || empty($_POST['slot']))
	{
	...
	else
	{
		...
		$slot = $db->Sanitize($_POST['slot']);
		$slot_code = $db->encryptor('decrypt',$_POST['slot_code']);
		$slot_code = $db->encryptor('decrypt',$slot_code);
		$uid = $db->Sanitize($slot_code);
		...
		{
			$update = $db->sql_query("UPDATE users SET private_slot='".$db->SanitizeForSQL($slot)."' 
			WHERE user_id='".$db->SanitizeForSQL($row['user_id'])."'");
			if($update)
			{
