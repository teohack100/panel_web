...
if(!isset($_GET['uid']) && !isset($_GET['ucode']) || empty($_GET['uid']) || empty($_GET['ucode'])){
	...
	if($user_id_2 == 1 || $user_level_2 == 'superadmin'){
		$chk_qry = "user_id!=1 AND user_id='".$db->SanitizeForSQL($uid)."' AND code='".$db->SanitizeForSQL($ucode)."' LIMIT 1";
	}elseif($user_level_2 == 'subadmin' || $user_level_2 == 'administrator'){
		$chk_qry = "user_id!=1 AND user_id='".$db->SanitizeForSQL($uid)."' AND code='".$db->SanitizeForSQL($ucode)."' AND upline='".$user_id_2."' AND user_level='reseller' LIMIT 1";
	}elseif($user_level_2 == 'reseller'){
		$chk_qry = "user_id!=1 AND user_id='".$db->SanitizeForSQL($uid)."' AND code='".$db->SanitizeForSQL($ucode)."' AND upline='".$user_id_2."' AND user_level='subreseller' LIMIT 1";
	}
