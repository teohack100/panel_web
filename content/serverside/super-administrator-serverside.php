...
if($user_id_2 == 1 || $user_level_2 == 'superadmin'){
	$sql = "SELECT user_id, user_name, code, duration, vip_duration, private_duration, is_connected, credits, user_level, status, upline FROM users WHERE 1=1 AND user_id!=1 AND is_active!=0 AND is_freeze!=1 AND status='live' AND user_level='superadmin' ";
}
...
while( $row = $db->sql_fetchrow($query) ) {
	...
	if($row['user_level'] == 'superadmin'){
		if($credits == 0)
		{
			$credits_label = '<span class="badge badge-info"><span class="fas fa-coins"></span> Unlimited Credit/s</span>';
		}else{
			$credits_label = '<span class="badge badge-info"><span class="fas fa-coins"></span> Unlimited Credit/s</span>';
		}
	}
