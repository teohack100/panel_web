...
$sql = "SELECT user_id, user_name, private_duration, credits, user_level, is_connected, status, upline FROM users WHERE user_id!=1 AND is_active!=0 AND is_freeze!=1  AND status='live' ORDER BY user_name ASC";
$query = $db->sql_query($sql) or die();
$totalData = $db->sql_numrows($query);
$totalFiltered = $totalData;
if($user_id_2 == 1 || $user_level_2 == 'superadmin'){
	$sql = "SELECT user_id, user_name, code, private_duration, credits, user_level, is_connected, status, upline FROM users WHERE 1=1 AND user_id!=1 AND is_active!=0  AND private_duration > 0 AND is_groupname='bulk' AND is_groupname!='superadmin' AND is_groupname!='administrator' AND is_groupname!='subadmin' AND is_groupname!='reseller' AND is_groupname!='subreseller' AND is_vip!=1 AND is_private=1 AND is_freeze!=1 AND status='live'";
}else{
	$sql = "SELECT user_id, user_name, code, private_duration, credits, user_level, is_connected, status, upline FROM users WHERE 1=1 AND user_id!=1 AND is_active!=0 AND is_freeze!=1 AND status='live' AND private_duration > 0 AND is_groupname='bulk' AND is_groupname!='superadmin' AND is_groupname!='administrator' AND is_groupname!='subadmin' AND is_groupname!='reseller' AND is_groupname!='subreseller' AND is_vip!=1 AND is_private=1 AND user_level!='superadmin' AND user_id!='".$user_id_2."' AND upline='".$user_id_2."' ";
}
