...
$sql = "SELECT user_id, user_name, duration, credits, user_level, is_connected, status, upline FROM users WHERE user_id!=1 AND is_active!=0 AND is_freeze!=1  AND status='live' ORDER BY user_name ASC";
...
while( $row = $db->sql_fetchrow($query) ) {
	...
	$dur3 = $db->calc_time($privateSeconds);
	$privateSecondsRemainder = $privateSeconds % 60;
	$dur4 = '<span class="live-countdown" data-seconds="'.$privateSeconds.'">&nbsp;<strong>'.$dur3['days'].'</strong> Day(s) | <strong>'.$dur3['hours'].'</strong> Hour(s) and <strong>'.$dur3['minutes'].'</strong> Minute(s) <strong>'.$privateSecondsRemainder.'</strong> Second(s)</span>';
