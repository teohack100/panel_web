...
$sql = "SELECT user_id, user_name, private_duration, credits, user_level, status, upline FROM users WHERE user_id!=1 AND is_active!=0 AND is_freeze!=1  AND status='live' ORDER BY user_name ASC";
$query = $db->sql_query($sql) or die();
$totalData = $db->sql_numrows($query);
$totalFiltered = $totalData;
if($user_id_2 == 1 || $user_level_2 == 'superadmin'){
	$sql = "SELECT user_id, user_name, code, private_duration, credits, user_level, status, upline FROM users WHERE 1=1 AND user_id!=1 AND is_active!=0  AND private_duration<1 AND is_groupname='bulk' AND is_groupname!='superadmin' AND is_groupname!='administrator' AND is_groupname!='subadmin' AND is_groupname!='reseller' AND is_groupname!='subreseller' AND is_vip!=1 AND is_private=1 AND is_freeze!=1 AND status='live'";
}else{
	$sql = "SELECT user_id, user_name, code, private_duration, credits, user_level, status, upline FROM users WHERE 1=1 AND user_id!=1 AND is_active!=0 AND is_freeze!=1 AND status='live' AND private_duration<1 AND is_groupname='bulk' AND is_groupname!='superadmin' AND is_groupname!='administrator' AND is_groupname!='subadmin' AND is_groupname!='reseller' AND is_groupname!='subreseller' AND is_vip!=1 AND is_private=1 AND user_level!='superadmin' AND user_id!='".$user_id_2."' AND upline='".$user_id_2."' ";
}
...
while( $row = $db->sql_fetchrow($query) ) {
	...
	if($row['private_duration'] == 0){
		$private_duration = "&nbsp;<strong>". $dur3['days'] . "</strong> Day(s) | <strong>" . $dur3['hours'] . "</strong> Hour(s) and <strong>" . $dur3['minutes'] . "</strong> Minutes Left.";
	}elseif($row['private_duration'] < 3600){
		$private_duration = "&nbsp;<strong>". $dur3['days'] . "</strong> Day(s) | <strong>" . $dur3['hours'] . "</strong> Hour(s) and <strong>" . $dur3['minutes'] . "</strong> Minutes Left.";	
	}else{
		$private_duration = "&nbsp;<strong>". $dur3['days'] . "</strong> Day(s) | <strong>" . $dur3['hours'] . "</strong> Hour(s) and <strong>" . $dur3['minutes'] . "</strong> Minute(s)";
	}
	
	$dur4 = $private_duration;
	...
	$nestedData[] = '<span class="badge badge-info"><span class="fas fa-user-shield"></span> '.$select_row['user_name'].'</span>';
