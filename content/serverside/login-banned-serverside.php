...
$sql = "SELECT * FROM login_banned_ip ORDER BY ip ASC";

$query = $db->sql_query($sql) or die();
$totalData = $db->sql_numrows($query);
$totalFiltered = $totalData;

$sql = "SELECT * FROM login_banned_ip WHERE 1=1 ";
if( !empty($requestData['search']['value']) ) {
	$sql.=" AND ( ip LIKE '%".$requestData['search']['value']."%' ";
...
while( $row = $db->sql_fetchrow($query) )
{
	...
	$nestedData[] = '<input type="checkbox" name="chk[]" class="chk-box" value="'.$id.'">';
	$nestedData[] = '<a href="javascript:void(0)" onclick="loginEditIP('.$row['id'].')">'.$ipaddress.'</a>';
	$nestedData[] = $attempts;
